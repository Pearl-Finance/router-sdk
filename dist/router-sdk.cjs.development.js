...
var dist = require('@uniswap/sdk-core/dist');
...
var ZERO_PERCENT = /*#__PURE__*/new dist.Percent(ZERO);
var ONE_HUNDRED_PERCENT = /*#__PURE__*/new dist.Percent(100, 100);
...
var ApproveAndCall = /*#__PURE__*/function () {
  ...
  ApproveAndCall.encodeCallPositionManager = function encodeCallPositionManager(calldatas) {
    ...
    if (calldatas.length == 1) {
      ...
      var encodedMulticall = dist$1.NonfungiblePositionManager.INTERFACE.encodeFunctionData('multicall', [calldatas]);
  ...
  ApproveAndCall.encodeAddLiquidity = function encodeAddLiquidity(position, minimalPosition, addLiquidityOptions, slippageTolerance) {
    ...
    if (isMint(addLiquidityOptions)) {
      return ApproveAndCall.INTERFACE.encodeFunctionData('mint', [{
        ...
        amount0Min: dist$1.toHex(amount0Min),
        amount1Min: dist$1.toHex(amount1Min),
      ...
      return ApproveAndCall.INTERFACE.encodeFunctionData('increaseLiquidity', [{
        ...
        amount0Min: dist$1.toHex(amount0Min),
        amount1Min: dist$1.toHex(amount1Min),
        tokenId: dist$1.toHex(addLiquidityOptions.tokenId)
      }]);
    }
  };
...
var MulticallExtended = /*#__PURE__*/function () {
  ...
  MulticallExtended.encodeMulticall = function encodeMulticall(calldatas, validation) {
    ...
    if (typeof validation === 'undefined') {
      return dist$1.Multicall.encodeMulticall(calldatas);
    ...
    if (typeof validation === 'string' && validation.startsWith('0x')) {
      ...
      var deadline = dist$1.toHex(validation);
...
function encodeFeeBips(fee) {
  return dist$1.toHex(fee.multiply(10000).quotient);
}
...
var PaymentsExtended = /*#__PURE__*/function () {
  ...
  PaymentsExtended.encodeUnwrapWETH9 = function encodeUnwrapWETH9(amountMinimum, recipient, feeOptions) {
    ...
    if (typeof recipient === 'string') {
      return dist$1.Payments.encodeUnwrapWETH9(amountMinimum, recipient, feeOptions);
    }

    if (!!feeOptions) {
      var feeBips = encodeFeeBips(feeOptions.fee);
      var feeRecipient = dist.validateAndParseAddress(feeOptions.recipient);
      return PaymentsExtended.INTERFACE.encodeFunctionData('unwrapWETH9WithFee(uint256,uint256,address)', [dist$1.toHex(amountMinimum), feeBips, feeRecipient]);
    } else {
      return PaymentsExtended.INTERFACE.encodeFunctionData('unwrapWETH9(uint256)', [dist$1.toHex(amountMinimum)]);
    }
  };
  ...
  PaymentsExtended.encodeSweepToken = function encodeSweepToken(token, amountMinimum, recipient, feeOptions) {
    // if there's a recipient, just pass it along
    if (typeof recipient === 'string') {
      return dist$1.Payments.encodeSweepToken(token, amountMinimum, recipient, feeOptions);
    }

    if (!!feeOptions) {
      var feeBips = encodeFeeBips(feeOptions.fee);
      var feeRecipient = dist.validateAndParseAddress(feeOptions.recipient);
      return PaymentsExtended.INTERFACE.encodeFunctionData('sweepTokenWithFee(address,uint256,uint256,address)', [token.address, dist$1.toHex(amountMinimum), feeBips, feeRecipient]);
    } else {
      return PaymentsExtended.INTERFACE.encodeFunctionData('sweepToken(address,uint256)', [token.address, dist$1.toHex(amountMinimum)]);
    }
  };

  PaymentsExtended.encodePull = function encodePull(token, amount) {
    return PaymentsExtended.INTERFACE.encodeFunctionData('pull', [token.address, dist$1.toHex(amount)]);
  };

  PaymentsExtended.encodeWrapETH = function encodeWrapETH(amount) {
    return PaymentsExtended.INTERFACE.encodeFunctionData('wrapETH', [dist$1.toHex(amount)]);
  };
...
var MixedRouteSDK = /*#__PURE__*/function () {
  ...
  _createClass(MixedRouteSDK, [{
    ...
    get: function get() {
      ...
      return this._midPrice = new dist.Price(this.input, this.output, price.denominator, price.numerator);
    }
  }]);
...
var MixedRouteTrade = /*#__PURE__*/function () {
  ...
  function MixedRouteTrade(_ref) {
    ...
    for (var _iterator = _createForOfIteratorHelperLoose(routes), _step; !(_step = _iterator()).done;) {
      ...
      for (var _iterator2 = _createForOfIteratorHelperLoose(route.pools), _step2; !(_step2 = _iterator2()).done;) {
        ...
        pool instanceof dist$1.Pool ? poolAddressSet.add(dist$1.Pool.getAddress(pool.token0, pool.token1, pool.fee)) : poolAddressSet.add(v2Sdk.Pair.getAddress(pool.token0, pool.token1));
      }
    }

    !(numPools == poolAddressSet.size) ?  invariant(false, 'POOLS_DUPLICATED')  : void 0;
    !(tradeType === dist.TradeType.EXACT_INPUT) ?  invariant(false, 'TRADE_TYPE')  : void 0;
    this.swaps = routes;
      ...
      return runtime_1.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 15:
              inputAmount = dist.CurrencyAmount.fromFractionalAmount(route.input, amount.numerator, amount.denominator);
              outputAmount = dist.CurrencyAmount.fromFractionalAmount(route.output, amounts[amounts.length - 1].numerator, amounts[amounts.length - 1].denominator);
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            ...
            case 3:
              ...
              !amount.currency.equals(route.input) ?  invariant(false, 'INPUT')  : void 0;
              inputAmount = dist.CurrencyAmount.fromFractionalAmount(route.input, amount.numerator, amount.denominator);
              amounts[0] = dist.CurrencyAmount.fromFractionalAmount(route.input.wrapped, amount.numerator, amount.denominator);
              i = 0;
  ...
  _proto.minimumAmountOut = function minimumAmountOut(slippageTolerance, amountOut) {
    ...
    var slippageAdjustedAmountOut = new dist.Fraction(ONE).add(slippageTolerance).invert().multiply(amountOut.quotient).quotient;
    return dist.CurrencyAmount.fromRawAmount(amountOut.currency, slippageAdjustedAmountOut);
  }
  /**
  ...
  _proto.worstExecutionPrice = function worstExecutionPrice(slippageTolerance) {
    return new dist.Price(this.inputAmount.currency, this.outputAmount.currency, this.maximumAmountIn(slippageTolerance).quotient, this.minimumAmountOut(slippageTolerance).quotient);
  }
  /**
        ...
        while (1) {
          switch (_context3.prev = _context3.next) {
            ...
            case 31:
              ...
              _context3.t1 = dist.sortedInsert;
              _context3.t2 = bestTrades;
              _context3.next = 36;
              return MixedRouteTrade.fromRoute(new MixedRouteSDK([].concat(currentPools, [pool]), currencyAmountIn.currency, currencyOut), currencyAmountIn, dist.TradeType.EXACT_INPUT);
  ...
  _createClass(MixedRouteTrade, [{
    ...
    get: function get() {
      ...
      }, dist.CurrencyAmount.fromRawAmount(inputCurrency, 0));
    ...
    get: function get() {
      ...
      var totalOutputFromRoutes = this.swaps.map(function (_ref7) {
        ...
        return total.add(cur);
      }, dist.CurrencyAmount.fromRawAmount(outputCurrency, 0));
    ...
    get: function get() {
      ...
      return (_this$_executionPrice = this._executionPrice) != null ? _this$_executionPrice : this._executionPrice = new dist.Price(this.inputAmount.currency, this.outputAmount.currency, this.inputAmount.quotient, this.outputAmount.quotient);
    }
    /**
    ...
    get: function get() {
      ...
      var spotOutputAmount = dist.CurrencyAmount.fromRawAmount(this.outputAmount.currency, 0);
      ...
      this._priceImpact = new dist.Percent(priceImpact.numerator, priceImpact.denominator);
      return this._priceImpact;
    }
  }]);
...
var RouteV3 = /*#__PURE__*/function (_V3RouteSDK) {
  ...
  return RouteV3;
}(dist$1.Route); // Mixed route wrapper
...
var Trade = /*#__PURE__*/function () {
  ...
  function Trade(_ref) {
    ...
    for (var _iterator = _createForOfIteratorHelperLoose(v2Routes), _step; !(_step = _iterator()).done;) {
      ...
      var _route = new RouteV2(routev2); // @ts-ignore
    ...
    for (var _iterator4 = _createForOfIteratorHelperLoose(this.swaps), _step4; !(_step4 = _iterator4()).done;) {
      ...
      for (var _iterator5 = _createForOfIteratorHelperLoose(_route3.pools), _step5; !(_step5 = _iterator5()).done;) {
        ...
        if (pool instanceof dist$1.Pool) {
          poolAddressSet.add(dist$1.Pool.getAddress(pool.token0, pool.token1, pool.fee));
  ...
  _proto.minimumAmountOut = function minimumAmountOut(slippageTolerance, amountOut) {
    ...
    if (this.tradeType === dist.TradeType.EXACT_OUTPUT) {
      return amountOut;
    } else {
      var slippageAdjustedAmountOut = new dist.Fraction(ONE).add(slippageTolerance).invert().multiply(amountOut.quotient).quotient;
      return dist.CurrencyAmount.fromRawAmount(amountOut.currency, slippageAdjustedAmountOut);
  ...
  _proto.maximumAmountIn = function maximumAmountIn(slippageTolerance, amountIn) {
    ...
    if (this.tradeType === dist.TradeType.EXACT_INPUT) {
      return amountIn;
    } else {
      var slippageAdjustedAmountIn = new dist.Fraction(ONE).add(slippageTolerance).multiply(amountIn.quotient).quotient;
      return dist.CurrencyAmount.fromRawAmount(amountIn.currency, slippageAdjustedAmountIn);
    }
  }
  /**
   * Return the execution price after accounting for slippage tolerance
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 5:
              ...
              _context.next = 9;
              return dist$1.Trade.fromRoute(routev3, _amount2, tradeType);
      ...
      return runtime_1.wrap(function _callee2$(_context2) {
        while (1) {
          switch (_context2.prev = _context2.next) {
            ...
            case 9:
              if (!(route instanceof dist$1.Route)) {
                _context2.next = 17;
                break;
              }

              _context2.next = 12;
              return dist$1.Trade.fromRoute(route, amount, tradeType);
  ...
  _createClass(Trade, [{
    ...
    get: function get() {
      ...
      return new dist.Percent(inputCurrency.wrapped.sellFeeBps.toNumber(), 10000);
    }
    /**
    ...
    get: function get() {
      ...
      return new dist.Percent(outputCurrency.wrapped.buyFeeBps.toNumber(), 10000);
    }
    /**
    ...
    get: function get() {
      ...
      for (var _iterator9 = _createForOfIteratorHelperLoose(this.swaps), _step9; !(_step9 = _iterator9()).done;) {
        ...
        var midPrice = route.midPrice;
        var postTaxInputAmount = inputAmount.multiply(new dist.Fraction(ONE).subtract(this.inputTax));
      ...
      var preTaxOutputAmount = this.outputAmount.divide(new dist.Fraction(ONE).subtract(this.outputTax));
  ...
  var _route$pools$reduce = route.pools.reduce(function (_ref, pool, index) {
    ...
    if (index === 0) {
      return {
        ...
        path: [inputToken.address, pool instanceof dist$1.Pool ? pool.fee : V2_FEE_PATH_PLACEHOLDER, outputToken.address]
      };
    } else {
      return {
        inputToken: outputToken,
        types: [].concat(types, ['uint24', 'address']),
        path: [].concat(path, [pool instanceof dist$1.Pool ? pool.fee : V2_FEE_PATH_PLACEHOLDER, outputToken.address])
      };
    }
  }, {
...
var partitionMixedRouteByProtocol = function partitionMixedRouteByProtocol(route) {
  ...
  while (right < route.pools.length) {
    if (route.pools[left] instanceof dist$1.Pool && route.pools[right] instanceof v2Sdk.Pair || route.pools[left] instanceof v2Sdk.Pair && route.pools[right] instanceof dist$1.Pool) {
...
var SwapRouter = /*#__PURE__*/function () {
  ...
  SwapRouter.encodeV3Swap = function encodeV3Swap(trade, options, routerMustCustody, performAggregatedSlippageCheck) {
    ...
    for (var _iterator = _createForOfIteratorHelperLoose(trade.swaps), _step; !(_step = _iterator()).done;) {
      ...
      var amountOut = dist$1.toHex(trade.minimumAmountOut(options.slippageTolerance, outputAmount).quotient); // flag for whether the trade is single hop or not
  ...
  SwapRouter.encodeSwaps = function encodeSwaps(trades, options, isSwapAndAdd) {
    ...
    if (trades instanceof Trade) {
      ...
      for (var _iterator3 = _createForOfIteratorHelperLoose(trades.swaps), _step3; !(_step3 = _iterator3()).done;) {
        ...
        if (route.protocol == exports.Protocol.V2) {
          individualTrades.push(new v2Sdk.Trade( // @ts-ignore
    ...
    var performAggregatedSlippageCheck = sampleTrade.tradeType === dist.TradeType.EXACT_INPUT && numberOfTrades > 2; // flag for whether funds should be send first to the router
  ...
  SwapRouter.swapAndAddCallParameters = function swapAndAddCallParameters(trades, options, position, addLiquidityOptions, tokenInApprovalType, tokenOutApprovalType) {
    ...
    var tokenOut = outputIsNative ? dist.WETH9[chainId] : positionAmountOut.currency.wrapped; // if swap output does not make up whole outputTokenBalanceDesired, pull in remaining tokens for adding liquidity
